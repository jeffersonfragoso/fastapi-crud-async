...
def test_create_note_invalid_json(test_app):
    ...
    response = test_app.post("/notes/", data=json.dumps({"title": "1",
                                                         "description": "2"}))
    assert response.status_code == 422
...
def test_read_all_notes(test_app, monkeypatch):
    ...
    async def mok_get_all():
        return test_data

    monkeypatch.setattr(crud, "get_all", mok_get_all)
...
def test_update_note(test_app, monkeypatch):
    test_update_data = {"title": "something",
    ...
    monkeypatch.setattr_put(crud, "put", mock_put)
